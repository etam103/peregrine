//! Minimal Weights & Biases client for logging training metrics and images.
...
impl WandbRun {
    ...
    pub fn init(project: &str) -> Self {
        match Inner::connect(project) {
            Some(inner) => {
                println!("wandb: https://wandb.ai/{}/{}/runs/{}\n",
    ...
    pub fn log_scalar(&mut self, key: &str, value: f32, step: usize) {
        if let Some(inner) = &mut self.inner {
            inner.append(serde_json::json!({ "_step": step, key: value }));
        }
    }
    ...
    pub fn log_image(&mut self, key: &str, rgb: &[u8], width: usize, height: usize, step: usize) {
        let inner = match &mut self.inner {
            Some(i) => i,
            None => return,
        };
        let img: RgbImage = match ImageBuffer::from_raw(width as u32, height as u32, rgb.to_vec()) {
            Some(i) => i,
            None => return,
        };
        ...
        let path = format!("media/images/{}_{}.png", key.replace('/', "_"), step);
        if inner.upload_file(&path, &png) {
            inner.append(serde_json::json!({
                "_step": step,
                key: {
                    "_type": "image-file",
                    "path": &path,
    ...
    pub fn finish(&mut self) {
        if let Some(inner) = &mut self.inner {
            ...
            println!("wandb: Run complete → https://wandb.ai/{}/{}/runs/{}",
                inner.entity, inner.project, inner.run_id);
        }
    }
}
...
impl Inner {
    fn connect(project: &str) -> Option<Self> {
        ...
        let resp = ureq::post(&format!("{}/graphql", API))
            ...
            .ok()?;
        let body: serde_json::Value = resp.into_json().ok()?;
        // Verify the mutation succeeded (no GraphQL errors)
    ...
    fn upload_file(&self, path: &str, data: &[u8]) -> bool {
        let resp = match ureq::post(&format!("{}/graphql", API))
            .set("Authorization", &self.auth)
            .send_json(serde_json::json!({
                "query": "mutation($e:String!,$p:String!,$r:String!,$f:[String!]!){createRunFiles(input:{entityName:$e,projectName:$p,runName:$r,files:$f}){files{name uploadUrl}}}",
                "variables": { "e": &self.entity, "p": &self.project, "r": &self.run_id, "f": [path] }
            })) {
            Ok(r) => r,
            Err(_) => return false,
        };
        ...
        let upload_url = match body["data"]["createRunFiles"]["files"][0]["uploadUrl"].as_str() {
            Some(u) => u.to_string(),
            None => return false,
        };
        ureq::put(&upload_url)
