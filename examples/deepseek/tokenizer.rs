...
impl Tokenizer {
    ...
    pub fn decode(&self, ids: &[usize]) -> String {
        ...
        let bytes: Vec<u8> = pieces
            .chars()
            .filter_map(|c| unicode_to_byte(c))
            .collect();
...
fn byte_to_unicode(b: u8) -> String {
    let c = match b {
        // Printable ASCII and some Latin-1 ranges map to themselves
        b'!'..=b'~' | 0xA1..=0xAC | 0xAE..=0xFF => b as u32,
        // Other bytes get shifted to avoid control characters
        _ => b as u32 + 256,
    };
...
fn parse_json_string(s: &str) -> Option<(String, usize)> {
    ...
    while let Some((i, c)) = chars.next() {
        match c {
            ...
            '"' => return Some((result, i + 2)), // +1 for opening quote, +1 for closing
            _ => result.push(c),
        }
    }
    None
...
fn parse_added_tokens(s: &str, tokens: &mut HashMap<String, usize>) {
    // Parse array of objects like {"id": N, "content": "...", ...}
    let mut pos = 0;
    let bytes = s.as_bytes();
    while pos < s.len() {
        if bytes[pos] == b'{' {
